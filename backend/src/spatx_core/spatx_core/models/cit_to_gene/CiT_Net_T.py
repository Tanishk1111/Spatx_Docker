...
class conv_block_DDConv(nn.Module):
    def __init__(self, ch_in, ch_out, device : str):
        super(conv_block_DDConv, self).__init__()
        self.device = device
        self.conv = nn.Sequential(
            DDConv(ch_in, ch_out, device= self.device, kernel_size=3, stride=1, padding=1),

            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True),

            DDConv(ch_out, ch_out, device= self.device, kernel_size=3, stride=1, padding=1),
...
class up_conv_DDConv(nn.Module):
    def __init__(self, ch_in, ch_out, device):
        super(up_conv_DDConv, self).__init__()
        self.device = device
        self.up = nn.Sequential(
            nn.Upsample(scale_factor=2),

            DDConv(ch_in, ch_out, device= self.device, kernel_size=1),
...
class CIT(nn.Module):
    def __init__(self, device : str, img_size=224, patch_size=4, in_chans=3, out_chans=1,
        ...
        self.Conv2e = conv_block_DDConv(ch_in=self.embed_dim * 1, ch_out=self.embed_dim * 2, device= self.device)
        self.Conv3e = conv_block_DDConv(ch_in=self.embed_dim * 2, ch_out=self.embed_dim * 4, device= self.device)
        self.Conv4e = conv_block_DDConv(ch_in=self.embed_dim * 4, ch_out=self.embed_dim * 8, device= self.device)
        self.Up4d = up_conv_DDConv(ch_in=self.embed_dim * 8, ch_out=self.embed_dim * 4, device= self.device)
      #  self.Up4d = up_conv_DDConv(ch_in=self.embed_dim * 8, ch_out=self.embed_dim * 8, device= self.device)
        self.Up_conv4d = conv_block(ch_in=self.embed_dim * 8, ch_out=self.embed_dim * 4)
       # self.Up_conv4d = conv_block(ch_in=self.embed_dim * 8 + self.embed_dim * 4, ch_out=self.embed_dim * 8)
        self.Up3d = up_conv_DDConv(ch_in=self.embed_dim * 4, ch_out=self.embed_dim * 2, device= self.device)
        self.Up_conv3d = conv_block(ch_in=self.embed_dim * 4, ch_out=self.embed_dim * 2)
        self.Up2d = up_conv_DDConv(ch_in=self.embed_dim * 2, ch_out=self.embed_dim * 1, device= self.device)
    ...
    def forward(self, x):   # 1,3,224,224
        ...
        Cnn5 = Cnn
        Swin5 = Swin
        #return torch.cat((Cnn5, Swin5), dim=1)
